...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      // гарантуємо наявність stock перед будь-якими операціями з gifts
      await ensureGiftStockColumn(env);
      ...
      // Рандом з УНІКАЛЬНІСТЮ: транзакція + stock + детальні логи
      if (url.pathname === '/api/random' && request.method === 'GET') {
        ...
        if (existing?.giftId) {
          console.log('random:already_assigned', existing.giftId);
        ...
        // глобальний KV-лок на короткий час, щоб мінімізувати гонки між IP
        const lockKey = `lock:random`;
        const gotLock = await tryLock(env, lockKey, 90);
        if (!gotLock) {
          console.warn('random:lock_busy');
          return json({ error: 'busy' }, corsHeaders, 429);
        }

        try {
          // повторна перевірка призначення (на випадок паралельних запитів)
          ...
          if (recheck?.giftId) {
            console.log('random:already_assigned_after_lock', recheck.giftId);
            const gift = await env.DB.prepare(
              "SELECT id, title, description, imageUrl FROM gifts WHERE id = ?"
          ...
          if (!dec.success) {
            console.warn('random:race_lost_on_update', pick.id);
            await env.DB.exec("ROLLBACK");
            return json({ error: 'race_lost' }, corsHeaders, 409);
          }

          // 3) створити/оновити assignment
          ...
          // ЖОДНИХ читань/повернень "duration". Фронт сам робить відлік.
          return json({ gift: pick }, corsHeaders);
        } catch (e) {
          console.error('random:error', e?.message || e);
          try { await env.DB.exec("ROLLBACK"); } catch (er2) { console.error('random:rollback_error', er2); }
  ...
  if (!giftSchemaReadyPromise) {
    giftSchemaReadyPromise = (async () => {
      try {
        ...
        console.warn('ensureGiftStockColumn: adding stock column due to error:', msg);
...
async function addStockColumn(env) {
  ...
  try {
    ...
    altered = true;
    console.log('addStockColumn: column stock added with DEFAULT 1');
  } catch (err) {
    const msg = String(err?.message || err || '');
    if (!/duplicate column|already exists/i.test(msg)) {
      console.error('addStockColumn: ALTER failed', msg);
      throw err;
    }
    console.log('addStockColumn: column already exists (race)');
  }
  if (altered) {
