...
function bindTabs() {
  $$('.tab').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      // автопідвантаження при перемиканні
      if (btn.dataset.tab === 'gifts') {
        await loadGifts();
      } else if (btn.dataset.tab === 'participants') {
        await loadAssigns();
      }
    });
  });
...
$('#btnLogin')?.addEventListener('click', async () => {
  ...
  if (r.ok) {
    ...
    await loadGifts();
    bindTabs(); // підв’язуємо після входу на всяк випадок
  } else {
...
async function loadGifts() {
  ...
  try {
    ...
    j = await r.json();
    console.log('Gifts:', j.items?.length ?? 0);
  } catch (e) {
...
async function loadAssigns() {
  ...
  try {
    ...
    j = await r.json();
    console.log('Assignments:', j.items?.length ?? 0);
  } catch (e) {
...
(function initAdmin() {
  ...
  // якщо токен уже є — одразу до адмінки
  if (token) {
    $('#loginView')?.classList.add('hidden');
    $('#adminView')?.classList.remove('hidden');
    loadGifts(); // дефолтна активна вкладка — Подарунки
  }
})();
