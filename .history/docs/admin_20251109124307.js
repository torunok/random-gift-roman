...
$('#btnLogin')?.addEventListener('click', async ()=>{
  ...
  if(r.ok){ const j=await r.json(); token=j.token; localStorage.setItem('admToken',token); $('#loginView').classList.add('hidden'); $('#adminView').classList.remove('hidden'); loadGifts(); loadAssigns(); }
  else { $('#loginMsg').textContent='Невірні дані'; }
});
...
async function loadGifts(){
  ...
  j.items.forEach(g=>{
    ...
    tr.innerHTML = `<td>${g.id}</td><td>${escapeHtml(g.title)}</td><td>${g.active? '✅' : '⛔'}</td>
      <td>
...
async function saveGift(){
  ...
  if(editGiftId){
    await fetch(API()+`/api/admin/gifts?id=${editGiftId}`,{method:'PUT',headers:setAuthHeader({'Content-Type':'application/json'}),body:JSON.stringify(payload)});
  } else {
    await fetch(API()+`/api/admin/gifts`,{method:'POST',headers:setAuthHeader({'Content-Type':'application/json'}),body:JSON.stringify(payload)});
  }
...
async function loadAssigns(){
  ...
  j.items.forEach(a=>{
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${a.id}</td><td>${escapeHtml(a.name||'')}</td><td>${escapeHtml(a.telegram||'')}</td><td>${a.giftId}</td><td>${a.createdAt}</td>
...
// Автовхід, якщо токен вже є
if(token){ $('#loginView').classList.add('hidden'); $('#adminView').classList.remove('hidden'); loadGifts(); loadAssigns(); }