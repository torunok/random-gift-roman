export default {
  async fetch(request, env, ctx) {
    ...
    // CORS одразу — щоб був доступний всюди
    ...
    try {
      ...
      // 1) Користувач погодився (зберігаємо ім'я, фіксуємо лог)
      if (url.pathname === '/api/agree' && request.method === 'POST') {
        ...
        // якщо ще немає рядка в assignments — створимо; якщо є й прийшло name — оновимо
      ...
      // 2) Статус мого IP (чи отримував подарунок)
      ...
      // 3) Рандом подарунка (1 раз на IP, з KV-локом)
      if (url.pathname === '/api/random' && request.method === 'GET') {
        const ipHash = await getIpHash(request, env.SALT);

        // якщо вже є призначення — віддаємо його
        ...
        try {
          // перечитати (раптом паралельний запит вже записав giftId)
      ...
      // Валідація JWT для всіх /api/admin/*
      ...
      if (url.pathname === '/api/admin/gifts') {
        ...
        if (request.method === 'DELETE') {
          const id = url.searchParams.get('id');
          // опційно: видалимо файл з R2, якщо зображення локальне через /img/
      ...
      // Assignments list/delete
      if (url.pathname === '/api/admin/assignments') {
        if (request.method === 'GET') {
          const q = url.searchParams.get('q');
          let sql = "SELECT id, name, telegram, giftId, createdAt FROM assignments";
          let params = [];
          if (q) {
      ...
      // ===== АДМІН: завантаження зображення у R2 =====
      if (url.pathname === '/api/admin/upload' && request.method === 'POST') {
        // Перевірка JWT вже зроблена вище (бо /api/admin/*)
        ...
        if (buf.byteLength > maxBytes) return json({ error: 'too_large' }, corsHeaders, 413);
        if (!allowed.includes(file.type)) return json({ error: 'bad_type' }, corsHeaders, 415);

        const ext = file.name.split('.').pop()?.toLowerCase() || mimeExt(file.type) || 'jpg';
      ...
      // Глобальна помилка — повернемо текст, щоб було що подивитись у фронті
...
function sanitizeStr(s) {
  if (s == null) return '';
  return String(s).slice(0, 2000); // простий обріз, щоб не ламати БД
}

async function readJson(request) {
  // страхуємося від порожнього body
